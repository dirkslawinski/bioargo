...
% <DS>
% - need to add checks for "T_CAL" for our cal files and "T_CAL_SW" from 
% the official ARGO document
      pats2check = {'CalTemp,', 'T_CAL', 'T_CAL_SW'};
      nPats = length(pats2check);
      for pdx = [1:nPats]
         if(~isempty(regexp(tline,  pats2check{pdx}, 'once')))
            fmt = sprintf('H,%s%%f', pats2check{pdx});
            cal.CalTemp = sscanf(tline, fmt, 1);
            break
         end % if T_CAL
      end % for pdx
